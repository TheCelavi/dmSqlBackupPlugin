...
class dmSqlBackupAdminActions extends dmAdminBaseActions {
    ...
    public function __construct($context, $moduleName, $actionName) {
        ...
        $this->backupService = $this->getService('sql_backup');
    }

    public function executeIndex(sfWebRequest $request) {
        if ($request->isMethod('post')) {
            if ($request->hasParameter('_backup') || $request->hasParameter('_backup_and_download')) $this->forward ('dmSqlBackupAdmin', 'backup');
            if ($request->hasParameter('_batch_delete')) $this->forward ('dmSqlBackupAdmin', 'batchDelete');
        }
        if ($this->getUser()->hasAttribute('dmSqlBackup/lastBackupFile')) {
            $this->backupFile = $this->getUser()->getAttribute('dmSqlBackup/lastBackupFile');
            $this->getUser()->getAttributeHolder()->remove('dmSqlBackup/lastBackupFile');
        }
    ...
    public function executeBackup(sfWebRequest $request) {        
        if (!$request->isMethod('post') || !($request->hasParameter('_backup') || $request->hasParameter('_backup_and_download'))) $this->forward ('dmSqlBackupAdmin', 'index'); 
        $this->form = new dmForm();
        if ($this->form->bindAndValid($request)) {            
            $backupFile = $this->backupService->execute(DmPageTable::getInstance()->getConnection());
            if ($backupFile) {                
                $this->getUser()->setFlash('notice', 'Backup created successfully!');
                if ($request->hasParameter('_backup_and_download')) {                    
                    $this->getUser()->setAttribute('dmSqlBackup/lastBackupFile', $backupFile);
                }
            } else {
        ...
        $this->forward ('dmSqlBackupAdmin', 'index'); 
    }

    public function executeBatchDelete(sfWebRequest $request) {
        if (!$request->isMethod('post') || !$request->hasParameter('_batch_delete')) $this->forward ('dmSqlBackupAdmin', 'index'); 
        $this->prepareFilesForForm(new dmForm());
    ...
    public function executeDelete(sfWebRequest $request) {
        if (!$request->isMethod('post') || !$request->hasParameter('_delete') || !$request->hasParameter('_file_name')) $this->forward ('dmSqlBackupAdmin', 'index');
        $this->form = new dmForm();        
    ...
    public function executeDownload(sfWebRequest $request) {
        if ($this->backupService->isBackUpExist($fileName = $request->getParameter('_file_name'))) {
            ...
            $response->setContentType('application/x-sql');
